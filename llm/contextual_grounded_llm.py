...
RESULTS_DIR = PROJECT_ROOT / "results" / "contextual_grounded"
...
# --------------------------------------------------
# Find contextual Terraform test cases
# --------------------------------------------------

CONTEXTUAL_DIR = PROJECT_ROOT / "testbed" / "contextual"
...
def find_test_cases():
    ...
    # Each contextual case is stored in its own folder.
    # Example:
    # C01/
    #   main.tf
    #   security.tf
    ...
    for case_dir in CONTEXTUAL_DIR.iterdir():

        if not case_dir.is_dir():
            continue

        terraform_files = sorted(case_dir.glob("*.tf"))

        if terraform_files:
            test_cases.append((case_dir.name, terraform_files))

    # Sort consistently: C01, C02, C03, ...
    test_cases.sort(key=lambda item: item[0])
...
def load_terraform(terraform_files):
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("SecureIaC - Contextual Grounded LLM Analysis")
    print("=" * 60)
    ...
    print(f"\nFound {len(test_cases)} contextual Terraform test cases:")

    for test_id, terraform_files in test_cases:
        print(f"  - {test_id}")
    ...
    for index, (test_id, terraform_files) in enumerate(test_cases):
        ...
        print("  Terraform files:")

        for terraform_file in terraform_files:
            print(f"    - {terraform_file.name}")

        terraform_code = load_terraform(terraform_files)
    ...
    print("\n" + "=" * 60)
    print("Contextual grounded analysis complete")
    print("=" * 60)
